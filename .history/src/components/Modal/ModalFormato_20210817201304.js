...
export const ModalFormato = ({isChangeModal, isSubmitModal}) => {
    ...
    const formatoCorrectoInv = (formato) => {
        if(formato === "2018"){return "Formato 2018/2019"}
        if(formato === "2020"){return "Formato 2019/2020"}
        if(formato === "2021"){return "Formato FMS Internacional"}
    }

    console.log(formato)
    console.log(formatoCorrectoInv())

    const [values, handleInputChange] = useForm({fms: formatoCorrectoInv(formato)});
    ...
    return (
        <div className="modal__container" onClick={handleModalDialogClick}>
            ...
            <select autoFocus name="formato" value={values.fms} className="modal__input cursor-p" onChange={handleInputChange}>
